...
func (model *RandomForest) Train(X [][]float64, y []float64, treeThreads int, featureThreads int) {
	model.Trees = make([]*DecisionTree, model.NumTrees)

	rs := make([]*rand.Rand, model.NumTrees)
	for i := range rs {
		source := rand.NewSource(int64(model.r.Intn(1000)))
		r := rand.New(source)
		rs[i] = r
	}
	var wg sync.WaitGroup
	wg.Add(model.NumTrees)
	sem := make(chan struct{}, treeThreads)

	trainRandomForestTree := func(i int) {
		defer wg.Done()

		sampledX, sampledY := bootstrapSample(X, y, rs[i])
		selectedFeatures := selectRandomFeatures(len(X[0]), model.MaxFeatures, rs[i])
		reducedX := filterXByFeatures(sampledX, selectedFeatures)
		model.Trees[i] = trainDecisionTree(reducedX, sampledY, 0, model.MaxDepth, selectedFeatures, featureThreads)
...
func trainDecisionTree(X [][]float64, y []float64, depth, maxDepth int, selectedFeatures []int, featureThreads int) *DecisionTree {
	...
	featureIndex, threshold := findBestSplit(X, y, featureThreads)
	...
	return &DecisionTree{
		...
		Threshold:    threshold,
		Left:         trainDecisionTree(leftX, leftY, depth+1, maxDepth, selectedFeatures, featureThreads),
		Right:        trainDecisionTree(rightX, rightY, depth+1, maxDepth, selectedFeatures, featureThreads),
	}
}
...
func findBestSplit(X [][]float64, y []float64, featureThreads int) (int, float64) {
	numFeatures := len(X[0])
	bestFeatureId := -1
	bestThreshold := 0.0
	bestGini := 1.0
	...
	wg.Add(numFeatures)
	// Semaphore
	sem := make(chan struct{}, featureThreads)
	// Mutex
	var mu sync.Mutex

	compareFeature := func(featureId int) {
		defer wg.Done()

		featureValues := make([]float64, len(X))
		for i := range X {
			featureValues[i] = X[i][featureId]
		}

		threshold := median(featureValues)
		leftIndices, rightIndices := splitX(X, featureId, threshold)
	...
	for featureId := 0; featureId < numFeatures; featureId++ {
		sem <- struct{}{}
		go compareFeature(featureId)
	}
	wg.Wait()
